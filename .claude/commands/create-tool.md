...
사용자가 `/[도구이름]`만 입력하거나 필요한 인자가 부족한 경우:
1. 도구 실행에 꼭 필요한 정보를 친절하게 질문합니다
2. 예시를 보여주며 안내합니다
3. 사용자가 답변하면 그때 실행을 진행합니다

**질문 예시:**
- "어떤 파일을 처리할까요? 예: `/[도구이름] example.md`"
- "변환할 파일 경로를 알려주세요."

**필요한 맥락이 모두 있는 경우에만** 아래 실행 단계로 진행합니다.

## 실행 단계

사용자가 `/[도구이름] [인자들]`을 실행하면:

1. 맥락 확인 → 부족하면 질문, 충분하면 진행
2. 설치 확인 (`which [스코프토큰]-[도구이름]`) → 없으면 자동 설치, 있으면 업데이트 확인
3. 인자 검증
4. 입력 파일 검색 (해당되는 경우)
5. 실행: `[스코프토큰]-[도구이름] [인자들]`
6. 결과 표시
